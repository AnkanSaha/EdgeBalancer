...
describe('LoadBalancerCard — analytics display', () => {
  it('shows skeleton loaders when analytics="loading"', () => {
    const { container } = render(<LoadBalancerCard {...DEFAULT_PROPS} showAnalytics analytics="loading" />);
  ...
  it('shows analytics stats when analytics data is provided', () => {
    render(<LoadBalancerCard {...DEFAULT_PROPS} showAnalytics analytics={{ requests: 1200, errors: 6, errorRate: 0.5 }} />);
    expect(screen.getByText(/1\.2k/)).toBeInTheDocument();
    expect(screen.getByText(/0\.5%/)).toBeInTheDocument();
  });

  it('formats request counts in millions correctly', () => {
    render(<LoadBalancerCard {...DEFAULT_PROPS} showAnalytics analytics={{ requests: 2_500_000, errors: 0, errorRate: 0 }} />);
    expect(screen.getByText(/2\.5M/)).toBeInTheDocument();
  });

  it('shows no analytics row when analytics=null', () => {
    render(<LoadBalancerCard {...DEFAULT_PROPS} showAnalytics analytics={null} />);
    expect(screen.queryByText(/req/)).not.toBeInTheDocument();
    expect(screen.queryByText(/err/)).not.toBeInTheDocument();
  });

  it('renders the error rate text when above 5%', () => {
    render(<LoadBalancerCard {...DEFAULT_PROPS} showAnalytics analytics={{ requests: 100, errors: 10, errorRate: 10 }} />);
  ...
  it('does not apply red color for error rate at or below 5%', () => {
    render(<LoadBalancerCard {...DEFAULT_PROPS} showAnalytics analytics={{ requests: 100, errors: 3, errorRate: 3 }} />);
  ...
  it('shows the Pro upsell instead of analytics for non-Pro users', () => {
    ...
    expect(screen.getByText(/Analytics available on paid plans/i)).toBeInTheDocument();
