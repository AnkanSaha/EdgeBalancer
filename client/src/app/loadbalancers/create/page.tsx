        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 400px', gap: 32, padding: 'clamp(16px, 4vw, 32px)', overflow: 'auto', flex: 1 }}>
          ...
          <FieldBlock n={5} title="Traffic Strategy *"
            ...
            <div className="strategy-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(clamp(160px, 50vw, 260px), 1fr))', gap: 'clamp(8px, 2vw, 10px)' }}>
              {STRATEGIES.map(s => {
                ...
                return (
                  <button key={s.id}
                    onClick={() => { if (!locked) { update('strategy', s.id); setActiveStep(5); } }}
                    style={{
                      textAlign: 'left', padding: 14,
                      borderRadius: 'var(--radius)',
                      border: `1px solid ${active ? 'var(--accent)' : locked ? 'var(--line)' : 'var(--line)'}`,
          ...
          <FieldBlock n={6} title="Traffic Handling"
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              }} onClick={() => { if (user?.isSubscribed) setActiveStep(6); }}>
                {!user?.isSubscribed && (
                  <div style={{
                    position: 'absolute', top: 8, right: 8,
                    display: 'flex', alignItems: 'center', gap: 4,
            ...
            {pathRoutingExpanded && (
              <div style={{ padding: '0 20px 16px', borderTop: '1px solid var(--line)' }}>
                <div style={{ fontSize: 12, color: 'var(--text-3)', margin: '12px 0' }}>
                  Route specific URL paths to dedicated origins. Requests not matching any rule use the default strategy above.
                </div>
