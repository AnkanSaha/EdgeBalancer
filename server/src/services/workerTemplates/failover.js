...
export default {
  async fetch(request, env) {
    ...
    // Path-based routing: direct to specific origin
    if (pathRoute) {
      const origin = config.origins[pathRoute.originIndex];
      return proxyToOrigin(origin, request);
    }

    // Default failover strategy: try each origin in order
    ...
    return lastResponse || new Response("All origin servers unavailable", { status: 502 });
  }
};
