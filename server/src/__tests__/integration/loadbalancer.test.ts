...
jest.mock('../../utils/redisClient', () => {
  // Stands in for the Lua script so the rate limiter is exercised, not skipped.
  const slots = new Map<string, number>();
  return {
    getRedisClient: jest.fn().mockResolvedValue({
      get: jest.fn().mockResolvedValue(null),
      set: jest.fn().mockResolvedValue('OK'),
      scriptLoad: jest.fn().mockResolvedValue('test-sha'),
      evalSha: jest.fn(async (_sha: string, opts: { keys: string[]; arguments: string[] }) => {
