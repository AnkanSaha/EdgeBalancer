import { validateBody, type ValidationFunction } from '../validation';
...
// Exported on its own so non-HTTP callers (the AI agent's create tool) can gate LLM-produced
// payloads through the exact same rules instead of keeping a second copy of them.
export const validateCreateLoadBalancerBody: ValidationFunction = (body) => {
  const errors: string[] = [];
...
export const createLoadBalancerValidator = [validateBody(validateCreateLoadBalancerBody)];
