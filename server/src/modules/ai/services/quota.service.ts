...
// A single Mistral model reporting its own limit is a much shorter-lived condition.
 ...
 */
export async function markProviderExhausted(provider: ModelProvider): Promise<void> {
  await write(providerKey(provider), PROVIDER_COOLDOWN_SECONDS);
}

export async function markModelExhausted(model: string): Promise<void> {
  await write(modelKey(model), MODEL_COOLDOWN_SECONDS);
}
